...
#End Region
    Public Function getSAPAccountcode(ByVal aCode As String) As String
        Dim oACTRS As SAPbobsCOM.Recordset
        oACTRS = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        oACTRS.DoQuery("Select isnull(AcctCode,'') from OACT where Formatcode='" & aCode & "'")
        Return oACTRS.Fields.Item(0).Value
    End Function
...
#Region "Linked Server Connection"
    Public Function linkedserverConnect() As Boolean
        Try
            LoginDetails()
            linkedserverDisconnect()
            Dim ORec, ORec2, ORec3 As SAPbobsCOM.Recordset
            ORec = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            ...
            ORec3 = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            str = "Select * from " & "[" & Server & "]" & "." & serverdb & "." & "dbo" & "." & "[SAP_Customer]"
            ORec3.DoQuery(str)
...
#Region "Linked Server  DisConnection"
    Public Sub linkedserverDisconnect()
        Try
            LoginDetails()
